...
const COMMAND_PALETTE_STATE = join(process.cwd(), "src/components/layout/command-palette-state.ts");

function readCommandPaletteSource() {
  return [COMMAND_PALETTE, COMMAND_PALETTE_STATE].map((file) => readFileSync(file, "utf8")).join("\n");
}

describe("command palette search-scope surface filtering", () => {
  it("uses isCmdkHrefAllowed for static and recent item filtering", () => {
    const raw = readCommandPaletteSource();
