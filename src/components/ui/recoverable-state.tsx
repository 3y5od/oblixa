...
function shouldShowV10RecoverableDiagnostics() {
  return (
    ...
    (process.env.NEXT_PUBLIC_SUPPORT_DIAGNOSTICS === "1" ||
      (process.env.NEXT_PUBLIC_SUPPORT_DIAGNOSTICS == null && process.env.NEXT_PUBLIC_V10_SUPPORT_DIAGNOSTICS === "1"))
  );
}

export function RecoverableState(props: {
  ...
  return (
    <section
      ...
      data-action={props.nextActionLabel ?? ""}
      data-v10-source-object={props.sourceObject ?? ""}
      data-source-object={props.sourceObject ?? ""}
      data-v10-diagnostic-id={props.diagnosticId ?? ""}
      data-diagnostic-id={props.diagnosticId ?? ""}
      data-v10-contract-ok={contractFailures.length === 0 ? "true" : "false"}
      data-contract-ok={contractFailures.length === 0 ? "true" : "false"}
      data-v10-focus-target="recoverable-state"
      data-focus-target="recoverable-state"
      data-v10-next-action-label={props.nextActionLabel ?? ""}
      data-next-action-label={props.nextActionLabel ?? ""}
        ...
        <div>
          ...
          {showDiagnostics ? (
            <p
              ...
              data-v10-contract-failures={contractFailures.join(",")}
...
/** @deprecated Use RecoverableState. Retained while route/page imports migrate. */
export const V10RecoverableState = RecoverableState;
