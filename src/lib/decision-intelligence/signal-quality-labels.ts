...
export type SignalQualityDisplayRow = { key: string; label: string; value: number };

/** @deprecated Use SignalQualityDisplayRow. The persisted metric keys remain v5-prefixed. */
export type V5SignalQualityDisplayRow = SignalQualityDisplayRow;

/** Turns merged JSON into sorted display rows (numeric values only). */
export function parseSignalQualityForDisplay(raw: unknown): SignalQualityDisplayRow[] {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return [];
  const obj = raw as Record<string, unknown>;
  const rows: SignalQualityDisplayRow[] = [];
...
/** @deprecated Use parseSignalQualityForDisplay. */
export const parseV5SignalQualityForDisplay = parseSignalQualityForDisplay;

// Version-name compatibility aliases. Prefer neutral exports in new code.
export { V5_SIGNAL_QUALITY_KEY_LABELS as SIGNAL_QUALITY_KEY_LABELS };
