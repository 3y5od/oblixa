...
describe("decision-packet-storage", () => {
  afterEach(() => {
    delete process.env.DECISION_PACKET_BUCKET;
  ...
  it("uploadDecisionPacketJsonArtifact returns null when storage API missing", async () => {
    process.env.DECISION_PACKET_BUCKET = "packets";
  ...
  it("prefers DECISION_PACKET_BUCKET while retaining legacy bucket fallback", async () => {
    process.env.DECISION_PACKET_BUCKET = "neutral-packets";
    process.env.V5_DECISION_PACKET_BUCKET = "legacy-packets";
    ...
    expect(from).toHaveBeenCalledWith("neutral-packets");
  });
