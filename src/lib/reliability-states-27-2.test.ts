...
import { V9_RELIABILITY_STATES } from "./compatibility-release-contract";
...
describe("V9 §27.2 reliability states map", () => {
  ...
  it("anchors job lifecycle copy + health diagnostics surfaces", () => {
    expect(readFileSync(join(process.cwd(), "src/lib/job-lifecycle-copy.ts"), "utf8").length).toBeGreaterThan(
      80
    );
  ...
  it("surfaces extraction job freshness via shared §27 relative-age helper", () => {
    ...
    expect(alert).toContain("@/lib/data-freshness");
  });
});
