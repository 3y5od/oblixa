...
function closureRows(): V10ZeroExclusionManifestRow[] {
  return buildV10CompleteClosureRows().map((row) => ({
    ...
    runtimeArtifact: row.proofArtifacts[0] ?? "src/lib/complete-closure.ts",
    ...
    rollbackPath: row.gates.find((gate) => gate.startsWith("npm run ")) ?? "npm run check:complete-closure",
...
function fixtureMeasurementRows(): V10ZeroExclusionManifestRow[] {
  const fixtureRows = V10_RC_FIXTURE_CATEGORIES.map((fixture) => ({
    ...
    runtimeArtifact: "src/lib/objective-measurements.ts",
    testArtifacts: ["src/lib/objective-measurements.test.ts", "npm run check:release-privacy-scan"],
    releaseEvidenceId: `v10-release:fixture-category:${fixture.category}`,
    blockerState: null,
    residualRisk: null,
    supportBoundary: "synthetic_fixture_data_only",
    rollbackPath: "npm run check:release-suite-current -- --cleanup-fixture all",
  ...
  const metricRows = V10_RC_METRIC_CAPTURE_PLANS.map((plan) => {
    ...
    return {
      ...
      runtimeArtifact: "src/lib/release-evidence.ts",
      testArtifacts: ["src/lib/release-evidence.test.ts", plan.captureCommand],
  ...
  const evidenceRows = V10_RELEASE_CANDIDATE_EVIDENCE_REQUIREMENTS.map((requirement) => ({
    ...
    rollbackPath: "npm run check:release-evidence",
...
function compatibilityRows(): V10ZeroExclusionManifestRow[] {
  const boundaryRows = V10_COMPATIBILITY_BOUNDARIES.map((boundary) => ({
    ...
    testArtifacts: ["src/lib/final-gap-audit.test.ts", "npm run check:release-suite-current"],
  ...
  const cleanupRows = V10_DEPRECATION_CLEANUP_DECISIONS.map((decision) => ({
    ...
    testArtifacts: [decision.cleanupCommand, "src/lib/final-gap-audit.test.ts"],
...
function driftRows(): V10ZeroExclusionManifestRow[] {
  return V10_POST_GA_DRIFT_CONTROLS.map((control) => ({
    ...
    runtimeArtifact: "src/lib/operational-contracts.ts",
    testArtifacts: ["src/lib/operational-contracts.test.ts", control.checkCommand],
...
function verificationRows(): V10ZeroExclusionManifestRow[] {
  ...
  const qualityRows = V10_QUALITY_MATRIX.map((row) => ({
    ...
    status: "automated_gate" as const,
    runtimeArtifact: "src/lib/operational-contracts.ts",
    testArtifacts: ["src/lib/operational-contracts.test.ts", "npm run test:e2e:current-product"],
    releaseEvidenceId: `v10-release:${row.evidenceKey}`,
    blockerState: null,
    residualRisk: null,
    supportBoundary: "accessibility_performance_browser_support",
    rollbackPath: "npm run check:release-suite-current",
    promotionState: "GA" as const,
  }));
...
function lifecycleRows(): V10ZeroExclusionManifestRow[] {
  const dataRows = V10_DATA_LIFECYCLE_COMPLIANCE_CONTRACTS.map((contract) => ({
    ...
    status: "automated_gate" as const,
    runtimeArtifact: "src/lib/operational-contracts.ts",
    testArtifacts: ["src/lib/operational-contracts.test.ts", contract.cleanupCommand],
...
function domainRows(): V10ZeroExclusionManifestRow[] {
  return V10_DOMAIN_COVERAGE_MATRICES.map((matrix) => ({
    ...
    runtimeArtifact: "src/lib/zero-exclusion-report.ts",
    testArtifacts: ["src/lib/zero-exclusion-report.test.ts"],
    releaseEvidenceId: matrix.primaryEvidenceKey,
    blockerState: null,
    residualRisk: null,
    supportBoundary: "domain_support_boundary",
    rollbackPath: "npm run check:complete-closure",
  ...
  return [
    {
      ...
      gateCommands: ["npm run check:release-suite-current", "npm run check:release-evidence"],
      supportSafe: true,
    },
    {
      packetKey: "GA",
      ...
      gateCommands: ["npm run check:complete-closure", "npm run test:e2e:current-product"],
      supportSafe: true,
    },
    {
      packetKey: "complete",
      ...
      gateCommands: ["npm run check:complete-closure", "npm run verify"],
      supportSafe: true,
    },
    {
      packetKey: "support",
      ...
      gateCommands: ["npm run check:release-evidence"],
      supportSafe: true,
    },
    {
    ...
    },
    {
      ...
      evidenceKeys: evidenceKeysFor("compatibility_boundary", "verification_gate"),
      gateCommands: ["npm run check:release-privacy-scan", "npm run check:release-evidence"],
      supportSafe: true,
    },
    {
      packetKey: "rollback",
...
function releaseHandoffRows(): V10ZeroExclusionManifestRow[] {
  return buildV10ReleaseHandoffPackets([]).map((packet) => ({
    ...
    status: "release_evidence_required",
    runtimeArtifact: "src/lib/zero-exclusion-report.ts",
    testArtifacts: ["src/lib/zero-exclusion-report.test.ts", ...packet.gateCommands],
    ...
    rollbackPath: packet.gateCommands[0] ?? "npm run check:release-evidence",
...
export { buildV10ReleaseHandoffPackets as buildReleaseHandoffPackets };
export { buildV10ZeroExclusionManifest as buildZeroExclusionManifest };
export { V10_DOMAIN_COVERAGE_MATRICES as DOMAIN_COVERAGE_MATRICES };
export { validateV10DomainCoverageMatrices as validateDomainCoverageMatrices };
export { validateV10ReleaseHandoffPackets as validateReleaseHandoffPackets };
export { validateV10ZeroExclusionManifest as validateZeroExclusionManifest };
export type { V10DomainCoverageMatrix as DomainCoverageMatrix };
export type { V10ReleaseHandoffPacket as ReleaseHandoffPacket };
export type { V10ZeroExclusionCategory as ZeroExclusionCategory };
export type { V10ZeroExclusionManifestRow as ZeroExclusionManifestRow };
export type { V10ZeroExclusionStatus as ZeroExclusionStatus };
