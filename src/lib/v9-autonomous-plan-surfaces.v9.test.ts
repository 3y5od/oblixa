...
describe("V9 autonomous plan — surface proxies (§7–§27 + matrices)", () => {
  ...
  describe("§21 loading + §22 errors", () => {
    it("anchors loading/error consistency + recoverable errors", () => {
      expect(existsSync(join(process.cwd(), "src/app/(dashboard)/loading-error-consistency.v9.test.ts"))).toBe(
        true
      );
      expect(read("src/lib/recoverable-mutation-error.ts").length).toBeGreaterThan(100);
      expect(read("src/lib/v9-api-client-errors.ts").length).toBeGreaterThan(80);
    });
  });

  describe("§23 performance + §27 reliability copy", () => {
    it("anchors page-load reporter + job lifecycle copy", () => {
      expect(read("src/components/layout/v9-page-load-reporter.tsx")).toContain("MEASURED_PREFIXES");
      expect(read("src/lib/v9-job-lifecycle-copy.ts").length).toBeGreaterThan(80);
      expect(read("src/lib/v9-data-freshness.ts").length).toBeGreaterThan(40);
    });
  });
  ...
  describe("§29.2 extended regression — security guard on disk", () => {
    it("retains v9-security-surface-guard for hidden-family + cmdk compliance", () => {
      expect(read("src/lib/v9-security-surface-guard.v9.test.ts").length).toBeGreaterThan(200);
    });
  });
