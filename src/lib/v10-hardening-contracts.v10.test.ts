...
describe("V10 final hardening contracts", () => {
  ...
  it("covers V10 dependency supply chain, deprecation cleanup, and support docs boundaries", () => {
    ...
    expect(
      validateV10SupplyChainScripts({
        "check:v10-suite": "node scripts/check-v10-suite.mjs",
        "check:v10-release-evidence": "node scripts/check-v10-release-evidence.mjs",
        "check:v10-privacy-scan": "node scripts/check-v10-release-evidence.mjs --privacy-scan activation",
    ...
    expect(validateV10SupplyChainScripts({ lint: "eslint src" })).toEqual(
      expect.arrayContaining(["missing_script:check:v10-suite", "missing_script:sbom"])
    );
  });
