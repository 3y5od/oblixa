...
const MANUAL_SMOKE_AUTOMATION_PROXIES: Record<SmokePath, readonly FileNeedle[]> = {
  ...
  "renewal clarification + seed playbook": [
    ["src/lib/acceptance-bundle.test.ts", "seedRenewalPlaybook"],
    ["src/lib/renewal-next-action.test.ts", "getRenewalNextAction"],
  ],
  ...
  "least-privilege vs editor on bulk export": [
    ["src/lib/export-csv-workspace-gate.test.ts", "requireApiWorkspaceEligibility"],
    ["src/app/api/import-export-job-org-scope.test.ts", "organization_id"],
  ],
  "import retry + evidence review — HTTP 429/413 user copy": [
    ["src/lib/api-client-errors.test.ts", "413"],
 ...
 * This bundle only pins matrix shape and links the tagged compatibility e2e anchor.
 */
describe("V9 manual smoke matrix — automation bridge (handoff proxy)", () => {
  ...
  it("keeps compatibility core smoke as the tagged regression anchor for overlapping journeys", () => {
    const smoke = readFileSync(join(process.cwd(), "e2e", "compatibility-core-smoke.spec.ts"), "utf8");
