...
describe("V9 §31–32 non-goals and refinement guardrails", () => {
  it("keeps trace rows for §31 and §32 anchored to constraint tests", () => {
    ...
    expect(s31).toContain("src/lib/global-constraints.test.ts");
    expect(s32).toContain("src/lib/spec-principles.test.ts");
  });

  it("constraint + principles harness files remain on disk", () => {
    for (const rel of ["src/lib/global-constraints.test.ts", "src/lib/spec-principles.test.ts"]) {
