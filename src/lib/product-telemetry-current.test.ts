...
import {
  ...
  normalizeProductTelemetryAction,
  PRODUCT_TELEMETRY_ACTIONS,
  PRODUCT_TELEMETRY_NEUTRAL_ACTION_ALIASES,
...
describe("V10 product telemetry", () => {
  ...
  it("accepts neutral aliases while persisting legacy compatibility labels", async () => {
    expect(normalizeProductTelemetryAction("product.approval_sla_breached")).toBe(
      "product.v10.approval_sla_breached"
    );
    expect(normalizeProductTelemetryAction("product.compat.visible_mutation_error")).toBe(
      "product.v9.visible_mutation_error"
    );
    expect(PRODUCT_TELEMETRY_NEUTRAL_ACTION_ALIASES["product.approval_sla_breached"]).toBe(
      "product.v10.approval_sla_breached"
    );
    ...
    const emitted = await emitProductTelemetryEvent(admin as never, {
      organizationId: "org_1",
      userId: "user_1",
      action: "product.approval_sla_breached",
    });
    ...
    expect(await emitProductTelemetryEvent(admin as never, {
      organizationId: "org_1",
      userId: "user_1",
      action: "product.not_registered",
    })).toBe(false);
  });

  it("normalizes every neutral product telemetry alias to a retained persisted event name", () => {
    const persistedActions = new Set<string>(PRODUCT_TELEMETRY_ACTIONS);

    for (const [neutralAction, persistedAction] of Object.entries(PRODUCT_TELEMETRY_NEUTRAL_ACTION_ALIASES)) {
      expect(neutralAction, neutralAction).not.toMatch(/^product\.v[0-9]+\./);
      expect(persistedActions.has(persistedAction), persistedAction).toBe(true);
      expect(normalizeProductTelemetryAction(neutralAction)).toBe(persistedAction);
    }
  });
});
