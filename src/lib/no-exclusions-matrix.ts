...
import { V10_NON_AUTONOMOUS_EVIDENCE_GATES } from "./release-evidence";
import { V10_ACCEPTANCE_MATRIX } from "./acceptance-matrix";
...
import { V10_AUTONOMOUS_COVERAGE_CONTRACTS, type V10CoverageContract } from "./autonomous-coverage";
import {
...
const NAVIGATION_FAMILY_ARTIFACTS: Record<(typeof V10_NAVIGATION_FAMILIES)[number], string> = {
  ...
  Advanced: "src/lib/advanced-assurance-continuity.ts",
  Assurance: "src/lib/advanced-assurance-continuity.ts",
};
...
export function buildV10NoExclusionsMatrix(): V10NoExclusionsMatrixRow[] {
  ...
  for (const requirement of V10_IMPLEMENTATION_REQUIREMENTS) {
    rows.push({
      ...
      sourceArtifacts: [...requirement.artifacts, "src/lib/implementation-checklist.ts"],
      testArtifacts: ["src/lib/implementation-checklist.test.ts", "npm run check:release-suite-current"],
  ...
  for (const contract of V10_AUTONOMOUS_COVERAGE_CONTRACTS) {
    ...
    rows.push({
      ...
      sourceArtifacts: [...contract.sourceArtifacts, "src/lib/autonomous-coverage.ts"],
      testArtifacts: ["src/lib/autonomous-coverage.test.ts", "src/lib/no-exclusions-matrix.test.ts"],
  ...
  for (const row of V10_SOURCE_INVENTORY) {
    rows.push({
      ...
      testArtifacts: row.testStatus === "release_check" ? ["src/lib/release-evidence.test.ts"] : ["src/lib/final-gap-audit.test.ts"],
  ...
  for (const family of V10_NAVIGATION_FAMILIES) {
    rows.push({
      ...
      testArtifacts: ["e2e/current-product-core-smoke.spec.ts", "src/lib/ui-state-contracts.test.ts"],
  ...
  for (const mutation of V10_REQUIRED_MUTATION_CONTRACTS) {
    rows.push({
      ...
      testArtifacts: ["src/lib/semantics.test.ts", "src/lib/route-api-catalog.test.ts"],
  ...
  for (const route of V10_ROUTE_API_CATALOG) {
    ...
    rows.push({
      ...
      sourceArtifacts: ["src/lib/route-api-catalog.ts", runtimeArtifact],
      testArtifacts: ["src/lib/route-api-catalog.test.ts", getV10RouteTestArtifact(route.path)],
  ...
  for (const modelKey of V10_REQUIRED_READ_MODEL_KEYS) {
    rows.push({
      ...
      sourceArtifacts: ["src/lib/read-models.ts", "src/lib/read-model-refresh.ts"],
      testArtifacts: ["src/lib/data-contracts.test.ts", "src/lib/read-model-refresh.test.ts"],
  ...
  for (const jobClass of V10_JOB_CLASSES) {
    rows.push({
      ...
      sourceArtifacts: ["src/lib/job-visibility.ts", "src/lib/read-model-refresh.ts"],
      testArtifacts: ["src/lib/read-model-refresh.test.ts", "src/lib/operational-contracts.test.ts"],
  ...
  for (const notificationClass of V10_NOTIFICATION_CLASSES) {
    rows.push({
      ...
      sourceArtifacts: ["src/lib/read-model-refresh.ts"],
      testArtifacts: ["src/lib/read-model-refresh.test.ts", "src/lib/operational-contracts.test.ts"],
  ...
  for (const reportFamily of V10_CORE_REPORT_FAMILIES) {
    rows.push({
      ...
      sourceArtifacts: ["src/lib/report-export.ts", "src/lib/read-model-refresh.ts"],
      testArtifacts: ["src/lib/report-export.test.ts", "src/lib/read-model-refresh.test.ts"],
  ...
  for (const state of V10_UI_STATE_CONTRACTS) {
    rows.push({
      ...
      sourceArtifacts: ["src/components/ui/recoverable-state.tsx", "src/lib/ui-state-contracts.ts"],
      testArtifacts: ["src/components/ui/recoverable-state.test.tsx", "src/lib/ui-state-contracts.test.ts"],
  ...
  for (const gate of V10_ACCEPTANCE_GATES) {
    ...
    rows.push({
      ...
      dimensions: ["test", "release_evidence", "operations"],
      sourceArtifacts: ["src/lib/acceptance-matrix.ts"],
      testArtifacts: ["src/lib/acceptance-matrix.test.ts"],
  ...
  for (const row of V10_COMPATIBILITY_BOUNDARIES) {
    rows.push({
      ...
      testArtifacts: ["src/lib/final-gap-audit.test.ts"],
  ...
  for (const metricKey of Object.keys(V10_GA_SAMPLE_SIZES)) {
    rows.push({
      ...
      sourceArtifacts: ["src/lib/objective-measurements.ts", "src/lib/objective-telemetry.ts", "scripts/check-release-evidence.mjs"],
      testArtifacts: ["src/lib/objective-measurements.test.ts", "src/lib/objective-telemetry.test.ts", "src/lib/release-evidence.test.ts"],
  ...
  for (const fixtureKey of Object.keys(V10_RELEASE_FIXTURE_MINIMUMS)) {
    rows.push({
      ...
      sourceArtifacts: ["src/lib/release-contract.ts", "scripts/check-release-suite-current.mjs"],
      testArtifacts: ["src/lib/release-evidence.test.ts"],
  ...
  for (const gate of V10_NON_AUTONOMOUS_EVIDENCE_GATES) {
    rows.push({
      ...
      dimensions: ["test", "release_evidence", "operations"],
      sourceArtifacts: ["src/lib/release-evidence.ts"],
      testArtifacts: ["src/lib/release-evidence.test.ts"],
  ...
  for (const boundary of V10_PROVIDER_BOUNDARIES) {
    rows.push({
      ...
      sourceArtifacts: ["src/lib/operational-contracts.ts"],
      testArtifacts: ["src/lib/operational-contracts.test.ts"],
  ...
  for (const action of V10_PRODUCT_TELEMETRY_ACTIONS) {
    rows.push({
      ...
      sourceArtifacts: ["src/lib/product-telemetry.ts", "src/lib/objective-telemetry.ts"],
      testArtifacts: ["src/lib/product-telemetry-current.test.ts", "src/lib/objective-telemetry.test.ts"],
  ...
  for (const audit of V10_AUDIT_VOCABULARY_TAXONOMY) {
    rows.push({
      ...
      sourceArtifacts: ["src/lib/final-gap-audit.ts", "src/lib/status-action-vocabulary.ts"],
      testArtifacts: ["src/lib/final-gap-audit.test.ts", "src/lib/status-action-vocabulary.test.ts"],
  ...
  for (const command of V10_REQUIRED_CI_RELEASE_COMMANDS) {
    rows.push({
      ...
      owner: "release",
      dimensions: ["test", "release_evidence", "operations"],
      sourceArtifacts: ["package.json", ".github/workflows/ci.yml", "scripts/check-release-suite-current.mjs"],
      testArtifacts: ["src/lib/release-evidence.test.ts", "src/lib/final-gap-audit.test.ts"],
...
export { buildV10NoExclusionsMatrix as buildNoExclusionsMatrix };
export { validateV10NoExclusionsMatrix as validateNoExclusionsMatrix };
export type { V10NoExclusionsCoverageKind as NoExclusionsCoverageKind };
export type { V10NoExclusionsCoverageStatus as NoExclusionsCoverageStatus };
export type { V10NoExclusionsDimension as NoExclusionsDimension };
export type { V10NoExclusionsMatrixRow as NoExclusionsMatrixRow };
