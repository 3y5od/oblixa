...
import { markNotificationSuppressed, deliverWithRetries } from "@/lib/notification-delivery";
...
import { executeBatch, safeErrorMessage, type BatchItemError } from "@/lib/route-runtime-contract";
import type { CronRouteHandlerContext, CronRouteHandlerResult } from "@/lib/cron/route-runner";
...
type AdminClient = CronRouteHandlerContext["admin"];
type SavedViewRow = {
  id: string;
  name: string;
  view_type: string;
...
async function refreshReportDeliveryReadModels(admin: AdminClient, organizationId: string, reason: string) {
...
export async function handleReportSummariesCron({
  admin,
}: CronRouteHandlerContext): Promise<CronRouteHandlerResult> {
  ...
  const buildSummary = async (
    ...
    if (savedView.view_type === "contracts") {
      ...
      if (deadlineIds !== null) {
        q = q.in("id", deadlineIds.length === 0 ? ["00000000-0000-0000-0000-000000000000"] : deadlineIds);
      }
      if (parsed.search) {
        q = q.or(
      ...
      if (atRiskResult.error) {
        errors.push(
          reportError(scope, "source_query", "report_summary_contract_exception_query_failed", atRiskResult.error.message)
        );
        return null;
      }
      ...
      if (overdueResult.error) {
        errors.push(
          reportError(scope, "source_query", "report_summary_obligation_exception_query_failed", overdueResult.error.message)
        );
        return null;
      }
        ...
        return [
          {
            ...
            meta: `${contract.title} · ${obligation.status}${
      ...
      if (blockerResult.error) {
        errors.push(
          reportError(scope, "source_query", "report_summary_renewal_exception_query_failed", blockerResult.error.message)
        );
        return null;
      }
  ...
  const processSubscription = async (subscription: ReportSubscriptionRow, profileByUserId: Map<string, string>) => {
    ...
    try {
      ...
      const orgSettings = await getOrgSettingsJson(admin, subscription.organization_id);
      const workspaceProductMode = orgSettings.workspace_mode;
      ...
      if (subscriptionUpdateError) {
        errors.push(reportError(subscription.id, "persist", "report_summary_subscription_update_failed", subscriptionUpdateError.message));
      }
  ...
  for (const organizationId of refreshOrganizations) {
    try {
      await refreshReportDeliveryReadModels(admin, organizationId, "report_delivery_batch");
    } catch (error) {
      errors.push(
        reportError(organizationId, "refresh", "report_summary_refresh_failed", safeErrorMessage(error) ?? "report summary refresh failed")
      );
    }
  }
