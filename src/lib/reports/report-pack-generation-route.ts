...
import { computeReportPackMetrics, extractPriorKpis } from "@/lib/contract-operations/report-pack-metrics";
...
export async function runSingleReportPackGeneration(input: {
  ...
  const orgSettings = await getOrgSettingsJson(input.admin, orgId);
  const workspaceProductMode = parseWorkspaceMode(orgSettings);
...
export async function handleReportPacksGenerateCron({
  admin,
