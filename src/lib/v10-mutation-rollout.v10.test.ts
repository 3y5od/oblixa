...
import { V10_MUTATION_CATALOG } from "./v10-release-contract";
import { V10_EXPECTED_VERSION_EXEMPT_MUTATIONS, V10_REQUIRED_MUTATION_CONTRACTS, validateV10RequiredMutationContracts } from "./v10-mutation-envelope";
import {
...
describe("V10 mutation rollout contracts", () => {
  ...
  it("requires rollback, backfill, rebuild, archive, and owner proof before GA rollout", () => {
    expect(existsSync(join(process.cwd(), "scripts/rebuild-v10-read-models.mjs"))).toBe(true);
    expect(
      validateV10RollbackReadiness({
        rollbackState: "ga_candidate",
        rollbackRunbook: "scripts/rebuild-v10-read-models.mjs",
  ...
  it("defines downgrade, migration safety, and repair checks for existing workspaces", () => {
    ...
    expect(V10_MIGRATION_SAFETY_CHECKS.find((check) => check.checkKey === "057_v10_runtime_contracts_forward_only")).toMatchObject({
      ...
      rollbackPath: "scripts/rebuild-v10-read-models.mjs --dry-run",
    });
