...
import { renderV9PrBodyRollup } from "@/lib/compatibility-pr-body-rollup";
...
describe("V9 PR body rollup renderer", () => {
  it("includes spec sections, green scripts, and test-anchor updates", () => {
    const body = renderV9PrBodyRollup({
      ...
      testAnchorUpdates: ["Updated `src/lib/compatibility-spec-trace-map.ts` for §24."],
    });
    ...
    expect(body).toContain("Test-anchor updates: Updated `src/lib/compatibility-spec-trace-map.ts` for §24.");
  });
