...
import { getV10ObjectiveMeasurementRule } from "./v10-objective-measurements";
import { buildV10SettingsHealthDiagnostics } from "./v10-governance";
...
describe("Phase 11 — human study + post-GA SLO governance (automatable contracts)", () => {
  ...
  it("clears external blocker review mode when no outstanding blockers are supplied", () => {
    const r = spawnSync("node", ["scripts/check-v10-release-evidence.mjs", "--external-blockers", "none"], {
      cwd: process.cwd(),
      encoding: "utf8",
    });
