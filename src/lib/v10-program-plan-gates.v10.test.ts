...
import { getV10AcceptanceMatrixRow } from "./v10-acceptance-matrix";
import { V10_REQUIRED_MUTATION_CONTRACTS, validateV10RequiredMutationContracts } from "./v10-mutation-envelope";
...
const INVENTORY_I_SCRIPT_KEYS = [
  ...
  "check:v10-inventory-lock",
  "check:v10-migration-smoke",
  "check:v10-migration-smoke:strict",
  "check:v10-promotable",
  "check:v10-promotable:report",
  "check:v10-runtime-evidence-plan",
  "check:v10-privacy-scan",
  "check:v10-zero-exclusion-report",
  "check:v10-complete-closure",
  "check:v10-suite",
  "rebuild:v10-read-models",
] as const;
...
describe("Phase 0 — baseline & Inventory I", () => {
  ...
  it("emits machine-parseable promotability JSON from check-v10-promotable --report", () => {
    const r = spawnSync("node", ["scripts/check-v10-promotable.mjs", "--report"], {
      cwd: process.cwd(),
      encoding: "utf8",
    });
...
describe("Phase 8 — P2 / advanced assurance continuity (in-branch scope)", () => {
  it("keeps advanced assurance continuity sources and regression tests present", () => {
    expect(existsSync(join(process.cwd(), "src/lib/v10-advanced-assurance-continuity.ts"))).toBe(true);
    expect(existsSync(join(process.cwd(), "src/lib/v10-continuity.v10.test.ts"))).toBe(true);
  });
});
...
describe("Phase 11 — post-GA evidence command surface", () => {
  ...
  it("emits structured JSON for --post-ga 7d (release_check_required)", () => {
    const r = spawnSync("node", ["scripts/check-v10-release-evidence.mjs", "--post-ga", "7d"], {
      cwd: process.cwd(),
      encoding: "utf8",
    });
  ...
  it("allows documented post-GA operator attestation when V10_POST_GA_EVIDENCE_CAPTURE_OK=1", () => {
    const r = spawnSync("node", ["scripts/check-v10-release-evidence.mjs", "--post-ga", "30d"], {
      cwd: process.cwd(),
      encoding: "utf8",
...
describe("Phase 12 — Semgrep + CI wiring", () => {
  it("keeps the V10 Semgrep pack and workflow checks", () => {
    ...
    expect(ci).toContain("semgrep/oblixa-v10-surface.yml");
    expect(ci).toContain("npm run check:v10-suite");
  });
});
