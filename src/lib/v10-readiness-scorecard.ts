...
export function buildV10OperationalHandoffArtifacts(): V10OperationalHandoffArtifact[] {
  ...
  const fromOps = (input: {
    ...
    return {
      ...
      verificationCommand: input.verificationCommand ?? contract?.rollbackCommand ?? "npm run check:v10-release-evidence",
  ...
  return [
    ...
    fromOps({ artifactKind: "release", opsKey: "slo_canary", runbookKey: "canary_hold", verificationCommand: "npm run check:v10-release-evidence" }),
    ...
    fromOps({ artifactKind: "provider_readiness", opsKey: "provider_readiness", runbookKey: "provider_outage" }),
    {
      ...
      verificationCommand: "npm run check:v10-release-evidence",
