...
import { listOrganizationIds } from "@/lib/assurance/cron";
import { runAssuranceChecks, recomputeScorecards } from "@/lib/assurance/service";
...
import { runControlPolicyReevaluation, runModularAssuranceChecks } from "@/lib/assurance/assurance-checks";
import { executeAutopilotAction, type AutopilotRuleRow } from "@/lib/assurance/autopilot-executors";
import { assembleReviewBoardPacket } from "@/lib/assurance/review-boards";
import { deliverReviewBoardRunNotifications } from "@/lib/assurance/review-board-notifications";
import { recomputeSegmentMemberships } from "@/lib/assurance/segments";
import { backfillOutcomeSnapshots } from "@/lib/assurance/outcome-writers";
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      if (result.errors.length > 0) {
        ...
        checkRuns += 1;
        await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_assurance_checks_org_ok_total", 1).catch(() => undefined);
      }
    } catch (err) {
      status = "failed";
...
export async function refreshFindingsAging(admin: AdminClient, providedOrgIds?: string[]) {
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_finding_refresh_org_processed_total", 1).catch(
        () => undefined
      );
    } catch (err) {
      status = "failed";
...
export async function runAutopilotDryRun(admin: AdminClient, providedOrgIds?: string[]) {
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      if (status === "success") {
        await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_autopilot_dry_run_org_ok_total", 1).catch(() => undefined);
      }
    } catch (err) {
      status = "failed";
...
export async function runAutopilotExecution(admin: AdminClient, providedOrgIds?: string[]) {
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      if (status === "success") {
        await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_autopilot_execution_org_ok_total", 1).catch(
          () => undefined
        );
      }
    } catch (err) {
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      if (result.error || (result.errors?.length ?? 0) > 0) {
        ...
        updated += 1;
        await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_scorecard_recompute_org_ok_total", 1).catch(
          () => undefined
        );
      }
    } catch (err) {
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      if ((res.errors ?? []).length > 0) {
        ...
        await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_health_graph_org_ok_total", 1).catch(() => undefined);
      }
    } catch (err) {
      status = "failed";
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      if (res.error) {
        ...
        evaluations += 1;
        await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_control_policy_reeval_org_ok_total", 1).catch(
          () => undefined
        );
      }
    } catch (err) {
...
export async function scanExternalWorkflowDeadlines(admin: AdminClient, providedOrgIds?: string[]) {
  ...
  for (const oid of orgsTouched) {
    await incrementAssuranceQualityCounter(admin, oid, "cron_v6_external_deadline_org_touched_total", 1).catch(() => undefined);
  }
...
export async function recomputeOutcomeEffectiveness(admin: AdminClient, providedOrgIds?: string[]) {
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      if (status === "success") {
        await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_outcome_effectiveness_org_ok_total", 1).catch(
          () => undefined
        );
      }
    } catch (err) {
...
export async function generateReviewBoardPackets(admin: AdminClient, providedOrgIds?: string[]) {
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      if (status === "success") {
        await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_review_board_packet_org_ok_total", 1).catch(
          () => undefined
        );
      }
    } catch (err) {
...
export async function recomputeSegmentMembershipsForAll(admin: AdminClient, providedOrgIds?: string[]) {
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      if (status === "success") {
        await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_segment_recompute_org_ok_total", 1).catch(
          () => undefined
        );
      }
    } catch (err) {
...
export async function runPlaybookFollowUpAssurancePasses(admin: AdminClient, providedOrgIds?: string[]) {
  ...
  for (const orgId of resolved.orgIds) {
    ...
    try {
      ...
      assuranceRuns += 1;
      await incrementAssuranceQualityCounter(admin, orgId, "cron_v6_playbook_followup_assurance_org_ok_total", 1).catch(
        () => undefined
      );
    } catch (err) {
      status = "failed";
