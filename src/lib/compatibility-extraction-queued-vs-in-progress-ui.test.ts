...
describe("suggestion queued vs in-progress UI copy", () => {
  it("anchors distinct user-facing labels for pending vs processing jobs", () => {
    ...
    expect(raw).toContain("Suggestions queued");
    expect(raw).toContain("Suggestions in progress");
    ...
    expect(buttonRaw).toContain("Suggestions queued");
