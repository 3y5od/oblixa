...
describe("review queue vs review cadence labels", () => {
  it("navigation exposes separate entries for the review queue and cadence workspace", () => {
    const nav = readFileSync(join(process.cwd(), "src/lib/navigation.ts"), "utf8");
    expect(nav).toMatch(/name:\s*"Review queue"/);
