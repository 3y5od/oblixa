...
import { buildExceptionFingerprint } from "@/lib/contract-operations/exceptions";
...
describe("buildExceptionFingerprint", () => {
  ...
  it("falls back to insert-then-update when a duplicate fingerprint already exists", async () => {
    ...
    const { upsertDetectedExceptions } = await import("@/lib/contract-operations/exceptions");
