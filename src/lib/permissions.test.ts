...
describe("canEditContracts", () => {
  it("allows owner, admin, editor, ops_manager, manager", () => {
    expect(canEditContracts("owner" as never)).toBe(true);
...
describe("canDeleteContracts", () => {
  it("allows only workspace owners and admins", () => {
    expect(canDeleteContracts("admin")).toBe(true);
    expect(canDeleteContracts("owner" as never)).toBe(true);
    expect(canDeleteContracts("manager")).toBe(false);
