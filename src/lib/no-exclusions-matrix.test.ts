...
import { V10_AUTONOMOUS_COVERAGE_CONTRACTS } from "./autonomous-coverage";
...
describe("V10 no-exclusions matrix", () => {
  it("generates complete runtime proof rows for every V10 coverage family", () => {
    ...
    expect(matrix.some((row) => row.coverageKind === "ci_release_gate" && row.coverageKey === "ci_release_gate:npm run check:complete-closure")).toBe(true);
    expect(matrix.some((row) => row.coverageKind === "ci_release_gate" && row.coverageKey === "ci_release_gate:npm run check:zero-exclusion-report")).toBe(true);
  ...
  it("makes mutable, searchable, and actionable source objects explicit", () => {
    ...
    expect(matrix.find((row) => row.coverageKey === "route:/api/command-palette/contracts:GET")).toMatchObject({
      ...
      testArtifacts: expect.arrayContaining(["src/app/api/command-palette/contracts/route.test.ts"]),
    });
    expect(matrix.find((row) => row.coverageKey === "route:/work:GET")).toMatchObject({
      ...
      testArtifacts: expect.arrayContaining(["e2e/current-product-core-smoke.spec.ts"]),
    });
    ...
    expect(matrix.find((row) => row.coverageKey === "ci_release_gate:npm run check:release-suite-current")).toMatchObject({
  ...
  it("blocks silent omissions and incomplete proof rows", () => {
    ...
    expect(validateV10NoExclusionsMatrix(incomplete)).toEqual(
      expect.arrayContaining([
        ...
        "missing_ci_release_gate:npm run check:release-suite-current",
        "missing_ci_release_gate:npm run check:release-inventory-lock",
