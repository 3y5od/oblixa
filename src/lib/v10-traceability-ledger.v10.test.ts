...
describe("V10 traceability ledger", () => {
      ...
      summarizeV10TraceabilityByStatus([
        {
          ...
          artifact: "test",
          status: "implemented",
          automatedProof: "npm run check:v10-suite",
  ...
  it("publishes doc-to-runtime proof rows joined to acceptance, CI, tests, and release evidence", () => {
    ...
    expect(proofRows.find((row) => row.specSection === "4.15")).toMatchObject({
      codeArtifacts: expect.arrayContaining(["src/lib/product-telemetry.ts", "src/lib/v10-release-contract.ts"]),
      testArtifacts: expect.arrayContaining(["src/lib/v10-objective-telemetry.v10.test.ts"]),
      acceptanceIds: expect.arrayContaining(["telemetry-objectives"]),
      ciChecks: expect.arrayContaining(["npm run check:v10-suite"]),
