...
const mockGetOrgSettingsJson = vi.hoisted(() => vi.fn());
const mockGetFeatureFlags = vi.hoisted(() => vi.fn());

vi.mock("@/lib/assurance/org-settings", () => ({
  getOrgSettingsJson: mockGetOrgSettingsJson,
}));
...
describe("loadProductSurfaceContext vs buildProductSurfaceContext (§8)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetOrgSettingsJson.mockResolvedValue(v6);
