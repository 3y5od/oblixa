...
describe("V8 §24 acceptance criteria (machine checks)", () => {
  describe("§24.1 governed surfaces mapped or exempt", () => {
    it("runs page, API, and action inventory gates", () => {
      const s = pkgScripts();
      expect(s["check:surface:page-inventory"]).toBeDefined();
      expect(s["check:surface:api-inventory"]).toBeDefined();
      expect(s["check:surface:action-inventory"]).toBeDefined();
    });
  });

  describe("§24.2 uniform eligibility", () => {
    it("includes acceptance matrix and API/action eligibility in v8-suite", () => {
      const suite = pkgScripts()["check:surface:suite"] ?? "";
      expect(suite).toContain("check:surface:acceptance-matrix");
      expect(suite).toContain("check:surface:api-eligibility");
      expect(suite).toContain("check:surface:action-eligibility");
    });
  });

  describe("§24.3 Core isolation", () => {
    it("runs strict href audit in v8-suite", () => {
      expect(pkgScripts()["check:surface:suite"]).toContain("check:surface:hrefs:strict");
    });
  });
  ...
  describe("§24.4 hidden modules", () => {
    it("includes vocabulary + refinement contracts in v8-suite", () => {
      const suite = pkgScripts()["check:surface:suite"] ?? "";
      expect(suite).toContain("check:surface:vocabulary");
  ...
  describe("§24.6 service-role discipline", () => {
    it("includes server-lib-admin in v8-suite", () => {
      expect(pkgScripts()["check:surface:suite"]).toContain("check:server-lib-admin");
    });
  });

  describe("§24.7 diagnostics", () => {
    it("includes diagnostics contract in v8-suite", () => {
      expect(pkgScripts()["check:surface:suite"]).toContain("check:surface:diagnostics-contract");
    });
  });
  ...
  describe("§24.8 suite fails on drift", () => {
    it("lists route inventory, denial mapping, and supplemental contracts", () => {
      ...
      expect(suite).toContain("check:surface:denial-mapping");
      expect(suite).toContain("check:surface:supplemental-contracts");
    });
  });
});
