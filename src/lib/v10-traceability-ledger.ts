...
import { V10_ACCEPTANCE_MATRIX, getV10AcceptanceProof } from "./v10-acceptance-matrix";
import {
...
function getLedgerAutomatedProof(id: (typeof LEDGER_IDS)[number]): string {
  if (id.includes("db") || id.includes("rls")) return "npm run check:migrations && src/lib/v10-data-contracts.v10.test.ts";
  if (id.includes("route") || id.includes("api") || id.includes("http-cache")) return "src/lib/v10-route-api-catalog.v10.test.ts";
  if (id.includes("ui") || id.includes("component")) return "src/lib/v10-ui-state-contracts.v10.test.ts";
  if (id.includes("readiness") || id.includes("evidence") || id.includes("risk")) return "src/lib/v10-release-evidence.v10.test.ts";
  if (id.includes("traceability") || id.includes("proof")) return "src/lib/v10-traceability-ledger.v10.test.ts";
  if (id.includes("command") || id.includes("search")) return "src/components/layout/command-palette.ui.test.tsx";
  return "npm run check:v10-suite";
}
...
function buildQueryableCoverageRow(input: {
  ...
  return {
    ...
    rollbackPath: "scripts/check-v10-suite.mjs",
    freshness: "fresh",
...
export function buildV10QueryableCoverageLedger(): V10QueryableCoverageLedgerRow[] {
  ...
  for (const gate of V10_ACCEPTANCE_GATES) {
    rows.push(buildQueryableCoverageRow({ coverageKind: "acceptance_gate", coverageKey: gate, sourceArtifact: "src/lib/v10-acceptance-matrix.ts" }));
  }
    ...
    for (const key of keys) {
      rows.push(
        buildQueryableCoverageRow({
          ...
          priority,
          owner: "product",
          sourceArtifact: "src/lib/v10-release-contract.ts",
        })
      );
    }
  }
  for (const route of V10_ROUTE_API_CATALOG) {
    rows.push(
      buildQueryableCoverageRow({
        ...
        sourceArtifact: "src/lib/v10-route-api-catalog.ts",
      })
    );
  }
  for (const mutation of V10_MUTATION_CATALOG) {
    rows.push(
      buildQueryableCoverageRow({
        ...
        priority: "P0",
        sourceArtifact: "src/lib/v10-mutation-rollout.ts",
      })
    );
    rows.push(
      buildQueryableCoverageRow({
  ...
  for (const key of V10_REQUIRED_READ_MODEL_KEYS) {
    rows.push(buildQueryableCoverageRow({ coverageKind: "read_model", coverageKey: key, priority: "P0", sourceArtifact: "src/lib/v10-read-models.ts" }));
  }
  ...
  for (const family of V10_CORE_REPORT_FAMILIES) {
    rows.push(buildQueryableCoverageRow({ coverageKind: "report_family", coverageKey: family, sourceArtifact: "src/lib/v10-report-export.ts" }));
  }
  return rows;
}
