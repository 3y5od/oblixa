...
vi.mock("@/lib/v6/policy-evaluator", () => ({
  evaluateSingleControlPolicy: (...args: unknown[]) => evaluateSingleControlPolicy(...args),
}));

vi.mock("@/lib/v6/service", async () => {
  const actual = await vi.importActual<typeof import("@/lib/v6/service")>("@/lib/v6/service");
  return {
    ...actual,
...
describe("generateControlPolicyReviewWork", () => {
  ...
  it("creates review tasks and evidence requirements for scoped published policy contracts", async () => {
    ...
    const { generateControlPolicyReviewWork } = await import("@/lib/v6/control-policies");
