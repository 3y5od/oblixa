...
import { recordV6AssuranceActivity } from "./telemetry";

describe("recordV6AssuranceActivity", () => {
  it("updates existing org_behavior_metrics row when present", async () => {
    ...
    await recordV6AssuranceActivity(admin, "org-1", { checks_run: 3 });
  ...
  it("inserts when no row for metrics_date", async () => {
    ...
    await recordV6AssuranceActivity(admin, "org-2", { foo: "bar" });
