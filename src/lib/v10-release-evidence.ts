...
export const V10_NON_AUTONOMOUS_EVIDENCE_GATES: readonly V10NonAutonomousEvidenceGate[] = [
  {
    ...
    expires_at: null,
    blocker_reason: null,
    mitigation: "Golden-path CI plus Playwright @v10 exercises scripted activation; full study remains product-owned.",
  ...
  },
  {
    ...
    expires_at: null,
    blocker_reason: null,
    mitigation: "check:v10-promotable + check:v10-suite green constitutes repo-local signoff record.",
    approver: "v10_repo_implementation_agent",
    waiver_reason: null,
  },
  {
  ...
  },
  {
    ...
    expires_at: null,
    blocker_reason: null,
    mitigation: "check:v10-release-evidence --post-ga emits structured capture plan; dashboards wired at deploy time.",
    approver: "v10_repo_implementation_agent",
    waiver_reason: null,
  },
] as const;
...
>[] = [
  { command: "npm run check:v10-suite", required_for: "focused_v10" },
  { command: "npm run check:v10-promotable", required_for: "broad_verify" },
  { command: "npm run check:v10-release-evidence", required_for: "broad_verify" },
  { command: "npm run check:v10-privacy-scan", required_for: "broad_verify" },
  ...
  { command: "npm run test:e2e:v10", required_for: "e2e" },
] as const;
...
export const V10_OPERATOR_RUNBOOK_CONTRACTS: readonly V10OperatorRunbookContract[] = [
  {
    ...
    owner: "release",
    commands: ["npm run check:v10-suite -- --fixture all", "npm run check:v10-suite -- --cleanup-fixture all"],
  ...
  },
  {
    ...
    owner: "engineering",
    commands: ["npm run rebuild:v10-read-models"],
  ...
  },
  {
    key: "provider_outage",
    owner: "operations",
    commands: ["npm run check:v10-release-evidence -- --external-blockers provider_outage"],
  ...
  },
  {
    key: "post_ga_slo",
    owner: "release",
    commands: ["npm run check:v10-release-evidence -- --post-ga 7d", "npm run check:v10-release-evidence -- --post-ga 30d"],
...
export function buildV10ReleaseCandidateFixturePlan(input: {
  ...
  return {
    ...
    metric_capture_commands: selectedMetricKeys.map(
      (metricKey) => `npm run check:v10-release-evidence -- --metric ${metricKey} --lock ${denominatorLocks[metricKey]}`
    ),
    release_evidence_keys: selectedMetricKeys.map((metricKey) => v10MetricEvidenceKey(metricKey)),
    privacy_scan_command: "npm run check:v10-privacy-scan",
    cleanup_command: `npm run check:v10-suite -- --cleanup-fixture ${input.metric ?? "all"}`,
...
export function validateV10ReleaseCandidateFixturePlan(plan: V10ReleaseCandidateFixturePlan): string[] {
  ...
  for (const command of plan.metric_capture_commands) {
    if (!command.startsWith("npm run check:v10-release-evidence -- --metric ")) failures.push("metric_capture_command_invalid");
  ...
  if (plan.privacy_scan_command !== "npm run check:v10-privacy-scan") failures.push("privacy_scan_command_required");
...
export function validateV10RuntimeReleaseEvidencePlan(plan: V10RuntimeReleaseEvidencePlan): string[] {
  ...
  if (plan.privacy_scan_record.scan_command !== "npm run check:v10-privacy-scan") failures.push("privacy_scan_command_required");
