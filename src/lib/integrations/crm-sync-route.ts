import type {
  CronRouteHandlerContext,
  CronRouteHandlerResult,
} from "@/lib/cron/route-runner";
...
async function enqueueEventSafely(
  errors: Array<Record<string, unknown>>,
    ...
    appendRouteError(errors, {
      ...
      message:
        error instanceof Error
          ? error.message
          : String(detail.message ?? "notification_failed"),
      organizationId:
        typeof detail.organizationId === "string"
      ...
      connectionId:
        typeof detail.connectionId === "string"
          ...
          : undefined,
      contractId:
        typeof detail.contractId === "string" ? detail.contractId : undefined,
    });
  }
}

export async function handleCrmSyncCron({
  admin,
  ...
  const orgIds = [
    ...new Set(connections.map((c) => String(c.organization_id))),
  ];
    ...
    orgIds.length === 0
      ...
      : await forEachSupabaseRangePage(
          ...
          async (chunk) => {
            ...
            const [
              { data: renewalScenarios, error: renewalError },
              { data: openExceptions, error: openExceptionsError },
            ] = await Promise.all([
            ...
            if (renewalError || openExceptionsError) {
              ...
              if (openExceptionsError) {
                appendRouteError(errors, {
                  diagnosticId:
                    "integrations_crm_exception_signals_load_failed",
                  phase: "source_query",
            ...
            const renewalByContract = new Map(
              (renewalScenarios ?? []).map((row) => {
                ...
                return [
                  typedRow.contract_id,
                  {
                    scenario: typedRow.scenario,
                    workspace_status: typedRow.workspace_status,
                  },
                ] as const;
              }),
            );
            const riskByContract = new Map<
              string,
              { openExceptions: number; criticalExceptions: number }
            >();
            for (const row of openExceptions ?? []) {
              if (!row.contract_id) continue;
              const current = riskByContract.get(String(row.contract_id)) ?? {
                openExceptions: 0,
            ...
            for (const contract of chunk) {
              ...
              const connection = connectionByOrg.get(
                contract.organization_id as string,
              );
              ...
              if (!cfg.endpointUrl) {
                ...
                await updateConnectionState(
                  ...
                  {
                    status: "error",
                    last_error: "Missing config_json.endpointUrl",
                  },
                  errors,
                  {
                    diagnosticId:
                      "integrations_crm_connection_error_write_failed",
                ...
                await updateContractState(
                  ...
                  {
                    diagnosticId:
                      "integrations_crm_contract_error_write_failed",
                ...
                await enqueueEventSafely(
                  ...
                  {
                    diagnosticId:
                      "integrations_crm_failure_event_enqueue_failed",
              ...
              if (!endpointUrl) {
                ...
                await updateConnectionState(
                  ...
                  {
                    status: "error",
                    last_error: "Invalid or unsafe config_json.endpointUrl",
                  },
                  errors,
                  {
                    diagnosticId:
              ...
              try {
                const timeoutMs = Math.min(
                  Math.max(Number(cfg.timeoutMs ?? 10000), 2000),
                  30000,
                );
                const risk = riskByContract.get(contractId) ?? {
                  openExceptions: 0,
                  criticalExceptions: 0,
                };
                ...
                if (!res.ok) {
                  ...
                  await updateConnectionState(
                    ...
                    {
                      status: "error",
                      last_error: `CRM sync failed: ${res.status}`,
                    },
                    errors,
                    {
                      diagnosticId:
                ...
                const wroteContract = await updateContractState(
                  ...
                  {
                    diagnosticId:
                      "integrations_crm_contract_success_write_failed",
                ...
                const wroteConnection = await updateConnectionState(
                  ...
                  {
                    diagnosticId:
                      "integrations_crm_connection_success_write_failed",
                ...
                if (wroteContract && wroteConnection) {
                  ...
                  await enqueueEventSafely(
                    ...
                    {
                      diagnosticId:
                        "integrations_crm_success_event_enqueue_failed",
