...
import {
  ...
  V9_NON_GOALS,
  V9_REGRESSION_GATES,
} from "./compatibility-release-contract";
...
describe("V9 plan enforcement bundles", () => {
  describe("§29.1 acceptance areas → concrete artifacts", () => {
    const rows: { area: string; files: string[] }[] = [
      ...
      },
      {
        ...
        files: ["src/lib/security-surface-guard.test.ts", "src/lib/product-surface/cmdk-search-jumps.ts"],
      },
      {
        area: "loading and mutation feedback quality",
        files: ["src/app/(dashboard)/loading-error-consistency.test.ts", "src/lib/recoverable-mutation-error.ts"],
      },
      { area: "major visible failure states", files: ["src/app/(dashboard)/error.tsx", "src/lib/api-client-errors.ts"] },
    ];
    ...
    const regressionAnchors: { area: string; tests: string[] }[] = [
      ...
      {
        area: "contract detail clarity and actions",
        tests: ["src/lib/contract-detail-summary.test.ts"],
      },
      {
        area: "review queue throughput path",
      ...
      {
        area: "hidden-feature non-leakage on visible surfaces",
        tests: ["src/lib/security-surface-guard.test.ts"],
      },
      {
        area: "loading and mutation feedback quality",
        tests: ["src/app/(dashboard)/loading-error-consistency.test.ts"],
      },
      { area: "major visible failure states", tests: ["src/lib/api-client-errors.test.ts"] },
    ];
  ...
  describe("§29.2 regression gates → package.json scripts", () => {
    it("maps four §29.2 regression gates to CI gate scripts (no silent deletion)", () => {
      ...
      expect(read("src/lib/reminder-delivery-visibility.test.ts").length).toBeGreaterThan(40);
      expect(read("src/lib/notification-diagnostics.ts")).toContain("Health");
    });
  });
  ...
  describe("§32 rollout metadata", () => {
    it("lists every NEXT_PUBLIC_V9_* toggle in v9-rollout.ts", () => {
      const body = read("src/lib/rollout.ts");
