...
describe("V10 core workflow contracts", () => {
      ...
      validateV10EndToEndDomainWorkflowContracts([
        {
          ...
          recoveryDestination: "/work",
          requiredProofs: ["src/lib/activation-state.ts"],
        },
      ])
    ).toEqual(
      expect.arrayContaining([
  ...
  it("keeps the Work hub empty queue slices on V10 recoverable-state contracts", () => {
    ...
    const workModel = readFileSync(join(process.cwd(), "src/lib/work/model.ts"), "utf8");
    const workSpecStrings = readFileSync(join(process.cwd(), "src/lib/work/spec-strings.ts"), "utf8");

    expect(workPage).toContain("loadWorkPageModel");
    expect(workModel).toContain("from(\"v10_work_items\")");
