...
import { V10_REQUIRED_PLAN_TODO_IDS } from "./v10-autonomous-coverage";
...
describe("V10 acceptance matrix", () => {
  it("keeps acceptance gates traceable to the autonomous planTodoId inventory (Phase 0 baseline)", () => {
    ...
    expect(
      V10_ACCEPTANCE_MATRIX.some((row) =>
        row.gates.includes("src/lib/v10-autonomous-coverage.v10.test.ts")
      )
    ).toBe(true);
    expect(V10_ACCEPTANCE_MATRIX.find((row) => row.id === "journey-contracts")?.gates).toEqual(
      expect.arrayContaining(["src/lib/v10-autonomous-coverage.v10.test.ts"])
    );
  });
  ...
  it("derives the full acceptance proof fields required for handoff", () => {
    ...
    expect(row ? getV10AcceptanceProof(row) : null).toMatchObject({
      ...
      verificationCommands: expect.arrayContaining(["npm run check:v10-suite"]),
    ...
    expect(row ? getV10AcceptanceProof(row).testGates : []).toEqual(
      expect.arrayContaining(["src/lib/v10-semantics.v10.test.ts"])
    );
  });
  ...
  it("keeps shipped claims tied to runtime artifacts and non-autonomous proof explicit", () => {
    ...
    expect(
      classifyV10AcceptanceRuntimeStatus({
        ...
        category: "release",
        disposition: "shipped",
        artifacts: ["src/lib/v10-implementation-checklist.ts", "src/lib/v10-final-gap-audit.ts"],
        gates: ["src/lib/v10-implementation-checklist.v10.test.ts"],
      })
        ...
        [
          {
            ...
            category: "release",
            disposition: "shipped",
            artifacts: ["src/lib/v10-implementation-checklist.ts"],
            gates: ["src/lib/v10-implementation-checklist.v10.test.ts"],
          },
        ],
        { requireAllIds: false }
  ...
  it("keeps npm gates and V10 browser smoke wired to real package scripts", () => {
    ...
    expect(scripts["test:e2e:smoke"]).toContain("e2e/v10-core-smoke.spec.ts");
    expect(scripts["test:e2e:v10"]).toContain("@v10");
  });
