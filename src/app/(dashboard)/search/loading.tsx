 ...
 *  shape-shift when data resolves: compact inline header, then a `lg` grid of
 *  an input + chip row + single band card (left) and a detail-rail card
 *  (right, lg only) matching the live `lg:grid-cols-[minmax(0,1fr)_21rem]`. */
export default function SearchLoading() {
  return (
    <div
      className="ui-page-stack mx-auto max-w-2xl gap-5 lg:max-w-6xl xl:max-w-[84rem]"
      role="status"
      aria-busy="true"
      aria-live="polite"
    ...
    >
      ...
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_21rem] lg:items-start lg:gap-6">
              ...
              <section key={bandIdx}>
                ...
                <ul>
                  {Array.from({ length: 3 }).map((__, rowIdx) => (
                    <li key={rowIdx} className="flex min-h-[44px] items-center gap-3 px-4 py-2">
        ...
        {/* Detail rail skeleton — desktop only, matches the live rail card */}
        <aside
          aria-hidden
          className="hidden self-start rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-raised)] p-4 shadow-[var(--shadow-1)] lg:block"
        >
          <span className="ui-skeleton block h-10 w-10 rounded-xl" />
          <span className="ui-skeleton mt-3 block h-4 w-2/3 rounded" />
          <div className="mt-2 flex gap-1.5">
            <span className="ui-skeleton h-4 w-14 rounded-full" />
            <span className="ui-skeleton h-4 w-12 rounded-full" />
          </div>
          <span className="ui-skeleton mt-3 block h-3 w-full rounded" />
          <span className="ui-skeleton mt-1.5 block h-3 w-4/5 rounded" />
          <span className="ui-skeleton mt-4 block h-9 w-full rounded-full" />
        </aside>
      </div>
    </div>
  );
