import type { getOrgSettingsJson } from "@/lib/assurance/org-settings";
...
import type { SettingsHealthDiagnosticsSectionsProps } from "./settings-health-diagnostics-sections";

type SettingsHealthAuthContext = NonNullable<Awaited<ReturnType<typeof getAuthContext>>>;
type SettingsHealthOrgSettings = Awaited<ReturnType<typeof getOrgSettingsJson>>;
...
export type SettingsHealthPageAction = {
  href: string;
  label: string;
};

export type SettingsHealthPageModel = {
  ...
  secondaryAffectedItems: WorkspaceHealthItem[];
  healthyItems: WorkspaceHealthItem[];
  heroCtaLabel: string;
  reportMetadataLabel: string;
  deliveryMetadataLabel: string;
  retryableImportJobId: string | null;
  latestFailedReportId: string | null;
  diagnosticsProps: SettingsHealthDiagnosticsSectionsProps;
};

type SettingsHealthPageModelInput = {
  admin: SettingsHealthAuthContext["admin"];
  orgId: SettingsHealthAuthContext["orgId"];
  orgSettings: SettingsHealthOrgSettings;
};
...
function buildHiddenFeatures(orgSettings: SettingsHealthOrgSettings): Set<string> {
...
export async function loadSettingsHealthPageModel({
  admin,
  orgId,
  orgSettings,
}: SettingsHealthPageModelInput): Promise<SettingsHealthPageModel> {
  const mode = parseWorkspaceHealthMode(orgSettings.workspace_mode);
  const hiddenFeatures = buildHiddenFeatures(orgSettings);
  ...
  const reviewReadyCount = countRows(contractRows, (row) =>
    Boolean((row as { required_next_step?: string | null }).required_next_step)
  );
  ...
  const openObligationCount = countRows(obligationRows, (row) =>
    ["open", "in_progress"].includes(String(row.status ?? ""))
  );
  const approvalRows = (approvalsRes.data ?? []) as StatusRow[];
  const pendingApprovalCount = countRows(approvalRows, (row) =>
    ["pending", "escalated"].includes(String(row.status ?? ""))
  );
  const exceptionRows = (exceptionsRes.data ?? []) as StatusRow[];
  const openExceptionCount = countRows(exceptionRows, (row) =>
    ["open", "in_progress"].includes(String(row.status ?? ""))
  );
  ...
  const activeMaintenanceCount = countRows(maintenanceRows, (row) =>
  ...
  const runningReportPackCount = countRows(reportPackRows, (row) =>
  ...
  return {
    ...
    primaryAction,
    primaryAffectedItem,
    secondaryAffectedItems,
    healthyItems,
    heroCtaLabel,
    reportMetadataLabel,
    deliveryMetadataLabel,
    retryableImportJobId: retryableImportJob ? String(retryableImportJob.id) : null,
    ...
    diagnosticsProps: {
      ...
      cronAuditEvents: (operationalEventsRes.data ?? []) as SettingsHealthDiagnosticsSectionsProps["cronAuditEvents"],
    },
  };
}
