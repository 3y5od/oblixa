...
describe("POST /api/assurance/external-links/[id]/response-pack", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns duplicate response before creating a response pack", async () => {
    ...
    const { POST } = await import("@/app/api/assurance/external-links/[id]/response-pack/route");
    const res = await POST(
      new Request("http://localhost/api/assurance/external-links/l1/response-pack", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-idempotency-key": "external-link-response-pack-replay-0001",
        },
        body: JSON.stringify({}),
      }),
