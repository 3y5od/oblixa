...
describe("POST /api/playbooks/[id]/run", () => {
  ...
  it("returns duplicate response before starting a playbook run", async () => {
    ...
    const { POST } = await import("@/app/api/playbooks/[id]/run/route");
    const res = await POST(
      new Request("http://localhost/api/playbooks/p1/run", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-idempotency-key": "playbook-run-replay-0001",
        },
        body: JSON.stringify({}),
      }),
