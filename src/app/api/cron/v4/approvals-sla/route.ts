...
export async function GET(request: Request) {
  ...
  const rate = await rateLimitCheck("cron:v4:approvals-sla", RATE_LIMITS.v4ApprovalSlaCron);
  if (!rate.ok) {
  ...
  const payload = { breaches, evaluated: rows.length, ok: true, durationMs: Date.now() - startedAt };
  pingCronHealthcheck("cron/v4/approvals-sla", payload);
