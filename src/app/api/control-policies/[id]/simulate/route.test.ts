...
describe("POST /api/control-policies/[id]/simulate", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns duplicate response before simulating a control policy", async () => {
    ...
    const { POST } = await import("@/app/api/control-policies/[id]/simulate/route");
    const res = await POST(
      new Request("http://localhost/api/control-policies/p1/simulate", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-idempotency-key": "control-policy-simulate-replay-0001",
        },
        body: JSON.stringify({}),
      }),
