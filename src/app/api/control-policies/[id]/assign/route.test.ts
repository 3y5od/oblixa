...
describe("POST /api/control-policies/[id]/assign", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns duplicate response before assigning a control policy", async () => {
    ...
    const { POST } = await import("@/app/api/control-policies/[id]/assign/route");
    const res = await POST(
      new Request("http://localhost/api/control-policies/p1/assign", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-idempotency-key": "control-policy-assign-replay-0001",
        },
        body: JSON.stringify({ targetId: "contract-1" }),
      }),
