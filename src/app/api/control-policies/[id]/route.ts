...
import { patchControlPolicySettings } from "@/lib/assurance/control-policies";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await incrementAssuranceQualityCounter(ctx.admin, ctx.orgId, "api_patch_control_policy_remediation_total", 1).catch(
    () => undefined
  );
