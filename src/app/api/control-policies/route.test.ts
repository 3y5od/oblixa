...
describe("POST /api/control-policies", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns duplicate response before creating a control policy", async () => {
    ...
    const res = await POST(
      new Request("http://localhost/api/control-policies", {
        ...
        headers: {
          ...
          "x-idempotency-key": "control-policy-create-replay-0001",
        },
        body: JSON.stringify({ name: "Policy", scope: "contracts" }),
      })
    );
