...
describe("POST /api/program-evolution/experiments/[id]/results", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns duplicate response before recording experiment results", async () => {
    ...
    const { POST } = await import("@/app/api/program-evolution/experiments/[id]/results/route");
    const res = await POST(
      new Request("http://localhost/api/program-evolution/experiments/e1/results", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-idempotency-key": "experiment-results-replay-0001",
        },
        body: JSON.stringify({}),
      }),
