...
import { simulateProgramEvolutionExperiment } from "@/lib/assurance/program-evolution";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await incrementAssuranceQualityCounter(ctx.admin, ctx.orgId, "api_post_program_evolution_simulate_total", 1).catch(
    () => undefined
  );
