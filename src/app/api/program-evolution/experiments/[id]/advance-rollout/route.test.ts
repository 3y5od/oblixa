...
describe("POST /api/program-evolution/experiments/[id]/advance-rollout", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns duplicate response before advancing rollout", async () => {
    ...
    const { POST } = await import("@/app/api/program-evolution/experiments/[id]/advance-rollout/route");
    const res = await POST(
      new Request("http://localhost/api/program-evolution/experiments/e1/advance-rollout", {
        method: "POST",
        headers: { "x-idempotency-key": "experiment-rollout-replay-0001" },
      }),
