...
describe("POST /api/review-boards", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns duplicate response before creating a review board", async () => {
    ...
    const { POST } = await import("@/app/api/review-boards/route");
    const res = await POST(
      new Request("http://localhost/api/review-boards", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-idempotency-key": "review-board-create-replay-0001",
        },
        body: JSON.stringify({ name: "Quarterly board" }),
      })
    );
