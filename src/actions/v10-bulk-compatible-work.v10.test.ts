...
import { V10_MUTATION_CATALOG, V10_WORK_ITEM_TYPES } from "@/lib/v10-release-contract";

const actionSource = readFileSync(new URL("./v10-bulk-compatible-work.ts", import.meta.url), "utf8");
const helperSource = readFileSync(new URL("./v10-bulk-compatible-work-helpers.ts", import.meta.url), "utf8");
...
describe("bulkAssignCompatibleV10WorkItems (V10)", () => {
  ...
  it("covers every non-specialized V10 work item type through the generic work-index path", () => {
    ...
    expect(actionSource).toContain("v10-bulk-compatible-work-helpers");
