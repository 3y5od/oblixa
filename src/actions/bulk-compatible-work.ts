...
import { type V10WorkItemStatus } from "@/lib/release-contract";
...
import {
  bulkAssignGenericV10WorkItems,
  bulkAssignObligationV10WorkItems,
  bulkCompleteGenericV10WorkItems,
  ensureOwnerOrgMember,
  isV10GenericBulkWorkItemType,
  MAX_BULK_V10_ITEMS,
  type V10BulkAssignWorkItemOutcome,
  type V10BulkWorkRow,
  V10_BULK_WORK_ITEM_SELECT,
} from "./bulk-compatible-work-helpers";

/**
 * Bulk-assign owners for rows in `v10_work_items` that share the same `compatible_action_group`.
 * Homogeneous batches only: specialized `contract_task`/`obligation` batches use source-table mutations;
 * every other V10 work item type is handled through the authoritative `v10_work_items` index.
 */
export async function bulkAssignCompatibleV10WorkItems(input: {
  ...
  const { data: rows } = await admin
    .from("v10_work_items")
    .select(V10_BULK_WORK_ITEM_SELECT)
    .in("id", ids);
  const workRows = (rows ?? []) as V10BulkWorkRow[];
  ...
  if (onlyType === "obligation" && workRows.every((r) => r.source_table === "contract_obligations")) {
    ...
    return bulkAssignObligationV10WorkItems({
      admin,
      organizationId,
      actorUserId: user.id,
      ids,
  ...
  if (isV10GenericBulkWorkItemType(onlyType)) {
    return bulkAssignGenericV10WorkItems({
      admin,
      organizationId,
      actorUserId: user.id,
      workRows,
  ...
  return { ok: false, error: `Bulk V10 assign does not support unknown work item type "${onlyType}".` };
}

/**
 * Bulk-complete work backed by `v10_work_items` (homogeneous batches; source-backed special cases stay delegated).
 */
export async function bulkCompleteCompatibleV10WorkItems(input: {
  ...
  if (onlyType === "obligation" && workRows.every((r) => r.source_table === "contract_obligations")) {
        ...
        const mutationOutcome =
          eligibleIds.length === obligationIds.length
            ...
            : eligibleIds.length > 0
              ? "dependency_blocked"
              : itemOutcomes.every((item) => item.outcome === "no_action")
                ? "no_action"
                : "validation_failed";
          ...
          message:
            mutationOutcome === "success"
              ...
              : mutationOutcome === "dependency_blocked"
                ...
                : mutationOutcome === "no_action"
                  ? "No obligation completion changes were needed."
    ...
    const ok = response.outcome === "success" || response.outcome === "dependency_blocked" || response.outcome === "no_action";
    return {
      ok,
  ...
  if (isV10GenericBulkWorkItemType(onlyType)) {
    return bulkCompleteGenericV10WorkItems({
      admin,
      organizationId,
      actorUserId: user.id,
      workRows,
  ...
  return { ok: false, error: `Bulk V10 complete does not support unknown work item type "${onlyType}".` };
}

// Version-name compatibility aliases. Prefer neutral exports in new code.
export { bulkAssignCompatibleV10WorkItems as bulkAssignCompatibleWorkItems };
export { bulkCompleteCompatibleV10WorkItems as bulkCompleteCompatibleWorkItems };
