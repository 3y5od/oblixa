...
function parseCatalogPaths() {
  const source = readFileSync(join(root, "src/lib/route-api-catalog.ts"), "utf8");
...
function classifyRouteBoundary(routePath) {
  const policies = [
    ...
    [/^\/api\/evidence\/requests\/\[[^\]]+\]\/remind\b/, "evidence", "core", "evidence_reminder_boundary"],
    ...
    [/^\/api\/security\/csp-report\b/, "settings", "core", "browser_security_report_boundary"],
