#!/usr/bin/env node
/**
 * Heuristic scan: hardcoded dashboard hrefs to advanced/assurance surfaces should appear with
 ...
 *
 * Scan scope: all of `src/app/(dashboard)` and `src/components` (excluding *.test.* / *.spec.*).
 * Native feature trees (in-family links expected): decisions, campaigns, assurance,
 * relationship-workspaces — entire files skipped (see NATIVE_ROUTE_MARKERS).
 */
...
/**
 * Destinations that must not appear unguarded outside native advanced/assurance trees.
 * Aligns with high-risk dashboard prefixes (compare routes match `decisions` / `campaigns` segments).
 */
const RISK_ALT =
...
const RISK_HREF_TEMPLATE_RE = new RegExp(
  `href=\\{\\s*\`\\/(${RISK_ALT})`,
  "m"
);
const RISK_PUSH_RE = new RegExp(
  `router\\.push\\(\\s*["'\`]\\/(${RISK_ALT})`,
  "m"
);
...
/** Repo-relative directory prefixes; files under these are native advanced/assurance trees. */
...
/**
 * Allowlist: exact path, suffix match, `prefix*` glob, or `some/dir/` directory prefix.
 */
...
function fileHasRisk(raw) {
  return (
    RISK_HREF_RE.test(raw) ||
    RISK_HREF_TEMPLATE_RE.test(raw) ||
    RISK_PUSH_RE.test(raw)
  );
}
...
if (violations.length === 0) {
  console.log("V7 cross-surface href audit passed.");
  process.exit(0);
}

console.error("V7 href audit: files with risky href patterns and no obvious eligibility guard token:");
for (const v of violations) console.error(` - ${v}`);
if (STRICT) {
  process.exit(1);
}
console.error("\nNon-strict: re-run with --strict to fail CI. Add false positives to scripts/v7-href-audit-allowlist.txt");
process.exit(0);
