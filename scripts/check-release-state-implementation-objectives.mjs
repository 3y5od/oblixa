...
function buildReport(config, scripts) {
  ...
  for (const row of routes) {
    ...
    if (row.disposition === "hide_for_release" && row.validationCommand !== "check:surface:hrefs:strict") {
  ...
  return {
    ...
    routeOwnerCounts,
    hiddenOrInternalRouteCount: routes.filter((row) => row.disposition === "hide_for_release" || row.disposition === "internal_only").length,
