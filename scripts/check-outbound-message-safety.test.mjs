...
test("analyzeOutboundMessageSafety validates mail/chat sanitization and header hygiene", () => {
  ...
  write(root, "src/lib/messaging/outbound-payload-scrub.ts", 'const SENSITIVE_KEY_RE = /token/i;\nconst QUERY_SECRET_RE = /token=/i;\nexport function redactOutboundMessageText() {}\nexport function sanitizeOutboundHtml() {}\nexport function scrubOutboundPayloadValue() {}\nexport function scrubOutboundMetadata() {}\n');
  write(root, "src/lib/messaging/outbound-payload-scrub.test.ts", 'it("redacts API keys, bearer tokens, cookies, signed URLs, and JWTs in text", () => {})\nit("redacts sensitive keys recursively while preserving safe metadata shape", () => {})\nit("removes active HTML content and dangerous URL schemes", () => {})\n');
  write(root, "src/lib/messaging/chat-snippet-sanitize.ts", 'import { redactOutboundMessageText } from "@/lib/messaging/outbound-payload-scrub";\nexport function sanitizeChatSnippet(text: string): string {\nreturn redactOutboundMessageText(text).replace(/@channel/gi, "@ channel").replace(/javascript:/gi, "javascript\\u200b:").replace(/<https?:\\/\\//gi, "<hxxp://");\n}\n');
  ...
  write(root, "src/lib/messaging/adaptive-card-snippet-sanitize.ts", 'import { redactOutboundMessageText } from "@/lib/messaging/outbound-payload-scrub";\nexport function sanitizeAdaptiveCardSnippet(jsonText: string): string {\nreturn redactOutboundMessageText(jsonText).replace(/javascript:/gi, "javascript\\u200b:").replace(/data:text\\/html/gi, "data\\u200b:text/html");\n}\n');
  ...
  write(root, "src/lib/messaging/discord-embed-snippet-sanitize.ts", 'import { redactOutboundMessageText } from "@/lib/messaging/outbound-payload-scrub";\nexport function sanitizeDiscordEmbedSnippet(text: string): string {\nreturn redactOutboundMessageText(text).replace(/@everyone/gi, "@ everyone").replace(/javascript:/gi, "javascript\\u200b:");\n}\n');
  write(root, "src/lib/messaging/discord-embed-snippet-sanitize.test.ts", 'it("defangs @everyone", () => {})\nit("breaks active content URL and script tokens", () => {})\nit("redacts token-shaped values in embed text", () => {})\n');
  write(root, "src/app/api/webhooks/dispatch/route.ts", 'import { scrubOutboundPayloadValue } from "@/lib/messaging/outbound-payload-scrub";\nconst safeEventPayload = scrubOutboundPayloadValue(event.payload ?? {}, {});\ndata: safeEventPayload,\n');
