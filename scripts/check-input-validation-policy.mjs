...
const REQUIRED_CI_COMMANDS = ["npm run check:input-validation-policy"];
const REQUIRED_SECURITY_PIPELINE_STEPS = ['"check:input-validation-policy"'];
const REQUIRED_FILE_MARKERS = {
  "src/lib/security/validation.ts": [
    "export function isUuid(",
    "export function isIsoDateOnly(",
    "export const JSON_UNSAFE_KEYS",
    "__proto__",
    "constructor",
    "prototype",
  ...
  "src/actions/field-comments-action-scope.test.ts": [
    'it("rejects unsafe comment text before contract lookup"',
  ],
  ...
  "src/actions/renewal-playbook-action-scope.test.ts": [
    'it("rejects unsafe note text before contract lookup"',
  ],
  "src/actions/watchlists.ts": [
    "MAX_WATCHLIST_NOTE_LEN",
    "allowTextWhitespaceControls: true",
  ],
  "src/actions/watchlists-action-scope.test.ts": [
    'it("upsertWatchlistEntryForm rejects unsafe note text before access queries"',
  ],
  ...
  "src/actions/notes-action-scope.test.ts": [
    'it("rejects unsafe note text before contract access lookup"',
  ],
...
const POSITIVE_INT_QUERY_PARAMS = new Set(["limit", "offset"]);
const FIXED_ENUM_QUERY_PARAMS = new Set(["artifact", "format", "role", "scope", "severity", "status"]);
const DATE_QUERY_PARAMS = new Set(["changed_since", "since"]);
const BOOLEAN_QUERY_PARAMS = new Set([
  "includeObligations",
  "includeReminders",
...
export function analyzeInputValidationPolicy(root = ROOT) {
  const issues = [];
  ...
  for (const abs of walkRoutes(path.join(root, "src", "app", "api"))) {
    const rel = path.relative(root, abs).replace(/\\/g, "/");
    const content = fs.readFileSync(abs, "utf8");
    if (/\[[^\]]+\]/.test(rel) && !content.includes("rejectUnsafeRouteParams(")) {
      issues.push({ issue: "dynamic_route_missing_param_shape_guard", rel });
    }
    if (rel.includes("/[action]/") && !content.includes("rejectInvalidRouteParamEnums(")) {
      issues.push({ issue: "dynamic_action_route_missing_enum_guard", rel });
    }
    for (const match of content.matchAll(/searchParams\.get\(\s*["'`]([^"'`]+)["'`]\s*\)/g)) {
      const param = match[1];
      if (POSITIVE_INT_QUERY_PARAMS.has(param) && !content.includes("parsePositiveIntParam(")) {
        issues.push({ issue: "query_param_missing_positive_int_parser", rel, param });
      }
      if (FIXED_ENUM_QUERY_PARAMS.has(param) && !content.includes("parseFixedEnumParam(")) {
        issues.push({ issue: "query_param_missing_fixed_enum_parser", rel, param });
      }
      if (
        DATE_QUERY_PARAMS.has(param) &&
        !content.includes("parseIsoTimestampParam(") &&
        !content.includes("parseIsoDateRange(")
      ) {
        issues.push({ issue: "query_param_missing_date_parser", rel, param });
      }
      if (BOOLEAN_QUERY_PARAMS.has(param) && !content.includes("parseBooleanParam(")) {
        issues.push({ issue: "query_param_missing_boolean_parser", rel, param });
      }
    }
  }

  return {
    checkId: "input-validation-policy",
    ok: issues.length === 0,
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  const report = analyzeInputValidationPolicy();
