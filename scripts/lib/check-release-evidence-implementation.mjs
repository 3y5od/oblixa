...
import { REQUIRED_V10_INDEXES } from "./current-required-indexes.mjs";
import { spawnSyncCrossPlatform } from "./cross-platform-spawn.mjs";
    ...
    JSON.stringify(
      {
        ...
        metricCaptureCommands: metrics.map(
          (metric) => `npm run check:release-evidence -- --metric ${metric} --lock ${denominatorLocks[metric]}`
        ),
        persistenceTables: [
        ...
        teardownCommand: `npm run check:release-suite-current -- --cleanup-fixture ${metricOption}`,
        syntheticDataUsedForPromotion: false,
    ...
    JSON.stringify(
      {
        ...
        privacyScanCommand: `npm run check:release-privacy-scan -- --privacy-scan ${metricCapture}`,
      },
      null,
      2
    )
...
const requiredFiles = [
  ...
  "src/lib/no-exclusions-matrix.ts",
  "src/lib/zero-exclusion-report.ts",
  "src/lib/source-object-inventory.ts",
  "src/lib/core-workflow-contracts.ts",
  "src/lib/product-telemetry.ts",
  "src/lib/server-contracts.ts",
  "src/lib/implementation-checklist.ts",
  "scripts/check-release-inventory-lock.mjs",
  "scripts/check-migration-smoke-current.mjs",
  "scripts/rebuild-read-models.mjs",
...
const productVersionTag = `v${10}`;
const legacyReleaseSuiteScript = `check:${productVersionTag}-suite`;
const legacyInventoryLockScript = `check:${productVersionTag}-inventory-lock`;
const legacyZeroExclusionScript = `check:${productVersionTag}-zero-exclusion-report`;
const legacyPrivacyScanScript = `check:${productVersionTag}-privacy-scan`;
const legacyMigrationSmokeScript = `check:${productVersionTag}-migration-smoke`;
const legacyReadModelScript = `rebuild:${productVersionTag}-read-models`;
const legacyCurrentProductE2eScript = `test:e2e:${productVersionTag}`;
if (packageJson.scripts?.[legacyReleaseSuiteScript] !== "npm run check:release-suite-current") {
  failures.push(`missing:${legacyReleaseSuiteScript}-alias`);
}
if (!packageJson.scripts?.["check:release-suite-current"]?.includes("scripts/check-release-suite-current.mjs")) {
  failures.push("missing:check:release-suite-current");
}
if (packageJson.scripts?.[legacyInventoryLockScript] !== "npm run check:release-inventory-lock") {
  failures.push(`missing:${legacyInventoryLockScript}-alias`);
}
if (!packageJson.scripts?.["check:release-inventory-lock"]?.includes("scripts/check-release-inventory-lock.mjs")) {
  failures.push("missing:check:release-inventory-lock");
}
if (packageJson.scripts?.[legacyZeroExclusionScript] !== "npm run check:zero-exclusion-report") {
  failures.push(`missing:${legacyZeroExclusionScript}-alias`);
}
if (!packageJson.scripts?.["check:zero-exclusion-report"]?.includes("src/lib/zero-exclusion-report.test.ts")) {
  failures.push("missing:check:zero-exclusion-report");
}
if (packageJson.scripts?.[legacyPrivacyScanScript] !== "npm run check:release-privacy-scan") {
  failures.push(`missing:${legacyPrivacyScanScript}-alias`);
}
if (!packageJson.scripts?.["check:release-privacy-scan"]?.includes("--privacy-scan all")) {
  failures.push("missing:check:release-privacy-scan-all");
}
if (packageJson.scripts?.[legacyMigrationSmokeScript] !== "npm run check:migration-smoke:current") {
  failures.push(`missing:${legacyMigrationSmokeScript}-alias`);
}
if (!packageJson.scripts?.["check:migration-smoke:current"]?.includes("scripts/check-migration-smoke-current.mjs")) {
  failures.push("missing:check:migration-smoke:current");
}
if (packageJson.scripts?.[legacyReadModelScript] !== "npm run rebuild:read-models") {
  failures.push(`missing:${legacyReadModelScript}-alias`);
}
if (!packageJson.scripts?.["rebuild:read-models"]?.includes("scripts/rebuild-read-models.mjs")) {
  failures.push("missing:rebuild:read-models");
}
if (packageJson.scripts?.[legacyCurrentProductE2eScript] !== "npm run test:e2e:current-product") {
  failures.push(`missing:${legacyCurrentProductE2eScript}`);
}
if (!packageJson.scripts?.["test:e2e:current-product"]?.includes("@current-product")) {
  failures.push("missing:test:e2e:current-product");
}
if (!packageJson.scripts?.["test:e2e:smoke"]?.includes("e2e/current-product-core-smoke.spec.ts")) {
...
for (const token of [
  "semgrep/oblixa-surface.yml",
  "npm run check:release-inventory-lock",
  "npm run check:release-suite-current",
  "npm run check:complete-closure",
  "npm run check:zero-exclusion-report",
...
const surfaceSemgrep = readFileSync(join(root, "semgrep/oblixa-surface.yml"), "utf8");
if ((surfaceSemgrep.match(/severity: ERROR/g) ?? []).length < 2) failures.push("surface-semgrep-error-severity-missing");

const ledger = readFileSync(join(root, "src/lib/traceability-ledger.ts"), "utf8");
...
if (!surfacePipeline.includes('"check:release-suite-current"')) failures.push("missing-surface-pipeline:v10-suite");
const v10SuiteScript = readFileSync(join(root, "scripts/check-release-suite-current.mjs"), "utf8");
...
const releaseEvidenceScript = readFileSync(join(root, "scripts/check-release-evidence.mjs"), "utf8");
...
const migrationSmokeScript = readFileSync(join(root, "scripts/check-migration-smoke-current.mjs"), "utf8");
for (const token of [
...
const mutationEnvelope = readFileSync(join(root, "src/lib/mutation-envelope.ts"), "utf8");
for (const token of [
...
const evidence = readFileSync(join(root, "src/lib/release-evidence.ts"), "utf8");
for (const symbol of [
...
const readModels = readFileSync(join(root, "src/lib/read-models.ts"), "utf8");
...
const autonomousCoverage = readFileSync(join(root, "src/lib/autonomous-coverage.ts"), "utf8");
for (const symbol of [
...
const finalGapAudit = readFileSync(join(root, "src/lib/final-gap-audit.ts"), "utf8");
for (const symbol of [
  ...
  const objectiveMeasurements = readFileSync(join(root, "src/lib/objective-measurements.ts"), "utf8");
...
const noExclusionsMatrix = readFileSync(join(root, "src/lib/no-exclusions-matrix.ts"), "utf8");
for (const symbol of [
...
const advancedAssuranceContinuity = readFileSync(join(root, "src/lib/advanced-assurance-continuity.ts"), "utf8");
for (const symbol of [
...
const operationalContracts = readFileSync(join(root, "src/lib/operational-contracts.ts"), "utf8");
for (const symbol of [
...
const sourceObjectInventory = readFileSync(join(root, "src/lib/source-object-inventory.ts"), "utf8");
for (const symbol of [
...
const coreWorkflows = readFileSync(join(root, "src/lib/core-workflow-contracts.ts"), "utf8");
for (const symbol of [
...
const routeCatalog = readFileSync(join(root, "src/lib/route-api-catalog.ts"), "utf8");
...
const serverContracts = readFileSync(join(root, "src/lib/server-contracts.ts"), "utf8");
...
const traceMap = readFileSync(join(root, "src/lib/spec-trace-map.ts"), "utf8");
